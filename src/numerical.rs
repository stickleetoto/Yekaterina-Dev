...
fn eval_x(e:&str,base:&HashMap<String,f64>,x:f64)->Result<f64,&'static str>{let mut vars=base.clone();vars.insert("x".to_string(),x);formula::eval(e,&vars)}
fn bisect(args:&[Value])->Result<Value,&'static str>{if args.len()<3||args.len()>5{return Err("ARG");}let(e,vars)=expr_obj(&args[0])?;let mut lo=num(&args[1])?;let mut hi=num(&args[2])?;let tol=if args.len()>=4{num(&args[3])?}else{1e-10};let max=if args.len()==5{args[4].as_u64().ok_or("TYPE")?.min(10_000) as usize}else{128};if !(tol>0.0)||lo>=hi{return Err("DOMAIN");}let mut flo=eval_x(e,&vars,lo)?;let fhi=eval_x(e,&vars,hi)?;if flo==0.0{return finite(lo);}if fhi==0.0{return finite(hi);}if flo.signum()==fhi.signum(){return Err("BRACKET");}for _ in 0..max{let mid=(lo+hi)*0.5;let fm=eval_x(e,&vars,mid)?;if fm.abs()<=tol||(hi-lo).abs()<=tol{return finite(mid);}if flo.signum()!=fm.signum(){hi=mid;}else{lo=mid;flo=fm;}}finite((lo+hi)*0.5)}
fn integrate_expr(args:&[Value])->Result<Value,&'static str>{if args.len()<3||args.len()>4{return Err("ARG");}let(e,vars)=expr_obj(&args[0])?;let a=num(&args[1])?;let b=num(&args[2])?;let n=if args.len()==4{args[3].as_u64().ok_or("TYPE")? as usize}else{1024};if n==0||n>MAX_STEPS{return Err("LIMIT");}let h=(b-a)/n as f64;let mut s=0.5*(eval_x(e,&vars,a)?+eval_x(e,&vars,b)?);for i in 1..n{s+=eval_x(e,&vars,a+h*i as f64)?;}finite(s*h)}
fn derivative_expr(args:&[Value])->Result<Value,&'static str>{if args.len()<2||args.len()>3{return Err("ARG");}let(e,vars)=expr_obj(&args[0])?;let x=num(&args[1])?;let h=if args.len()==3{num(&args[2])?}else{1e-5_f64*(1.0+x.abs())};if h==0.0{return Err("DIV0");}finite((eval_x(e,&vars,x+h)?-eval_x(e,&vars,x-h)?)/(2.0*h))}
fn newton(args:&[Value])->Result<Value,&'static str>{if args.len()<2||args.len()>4{return Err("ARG");}let(e,vars)=expr_obj(&args[0])?;let mut x=num(&args[1])?;let tol=if args.len()>=3{num(&args[2])?}else{1e-10};let max=if args.len()==4{args[3].as_u64().ok_or("TYPE")?.min(10_000) as usize}else{64};if tol<=0.0{return Err("DOMAIN");}for _ in 0..max{let fx=eval_x(e,&vars,x)?;if fx.abs()<=tol{return finite(x);}let h=1e-6_f64*(1.0+x.abs());let d=(eval_x(e,&vars,x+h)?-eval_x(e,&vars,x-h)?)/(2.0*h);if d.abs()<1e-15{return Err("DERIV");}let nx=x-fx/d;if !nx.is_finite(){return Err("NONFINITE");}if (nx-x).abs()<=tol{return finite(nx);}x=nx;}finite(x)}
