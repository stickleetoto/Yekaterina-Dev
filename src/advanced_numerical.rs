...
fn eval1(e:&str,base:&HashMap<String,f64>,x:f64)->Result<f64,&'static str>{let mut v=base.clone();v.insert("x".into(),x);formula::eval(e,&v)}
fn eval2(e:&str,base:&HashMap<String,f64>,x:f64,y:f64)->Result<f64,&'static str>{let mut v=base.clone();v.insert("x".into(),x);v.insert("y".into(),y);formula::eval(e,&v)}
fn secant(args:&[Value])->Result<Value,&'static str>{if args.len()<3||args.len()>5{return Err("ARG");}let(e,v)=expr(&args[0])?;let mut x0=num(&args[1])?;let mut x1=num(&args[2])?;let tol=if args.len()>=4{num(&args[3])?}else{1e-10};let max=if args.len()==5{args[4].as_u64().ok_or("TYPE")?.min(100_000) as usize}else{100};if tol<=0.0{return Err("DOMAIN");}for _ in 0..max{let f0=eval1(e,&v,x0)?;let f1=eval1(e,&v,x1)?;let d=f1-f0;if d.abs()<1e-18{return Err("DOMAIN");}let x2=x1-f1*(x1-x0)/d;if (x2-x1).abs()<=tol{return finite(x2);}x0=x1;x1=x2;}Err("NO_CONVERGE")}
fn false_position(args:&[Value])->Result<Value,&'static str>{if args.len()<3||args.len()>5{return Err("ARG");}let(e,v)=expr(&args[0])?;let mut a=num(&args[1])?;let mut b=num(&args[2])?;let tol=if args.len()>=4{num(&args[3])?}else{1e-10};let max=if args.len()==5{args[4].as_u64().ok_or("TYPE")?.min(100_000) as usize}else{100};let mut fa=eval1(e,&v,a)?;let mut fb=eval1(e,&v,b)?;if fa*fb>0.0{return Err("DOMAIN");}for _ in 0..max{let d=fb-fa;if d.abs()<1e-18{return Err("DOMAIN");}let c=(a*fb-b*fa)/d;let fc=eval1(e,&v,c)?;if fc.abs()<=tol{return finite(c);}if fa*fc<0.0{b=c;fb=fc;}else{a=c;fa=fc;}}Err("NO_CONVERGE")}
fn second_derivative(args:&[Value])->Result<Value,&'static str>{if args.len()<2||args.len()>3{return Err("ARG");}let(e,v)=expr(&args[0])?;let x=num(&args[1])?;let h=if args.len()==3{num(&args[2])?}else{1e-4};if h<=0.0{return Err("DOMAIN");}finite((eval1(e,&v,x+h)?-2.0*eval1(e,&v,x)?+eval1(e,&v,x-h)?)/(h*h))}
fn integrate_midpoint(args:&[Value])->Result<Value,&'static str>{if args.len()!=4{return Err("ARG");}let(e,v)=expr(&args[0])?;let a=num(&args[1])?;let b=num(&args[2])?;let n=args[3].as_u64().ok_or("TYPE")? as usize;if n==0||n>1_000_000{return Err("LIMIT");}let h=(b-a)/n as f64;let mut s=0.0;for i in 0..n{s+=eval1(e,&v,a+(i as f64+0.5)*h)?;}finite(s*h)}
fn integrate_simpson(args:&[Value])->Result<Value,&'static str>{if args.len()!=4{return Err("ARG");}let(e,v)=expr(&args[0])?;let a=num(&args[1])?;let b=num(&args[2])?;let n=args[3].as_u64().ok_or("TYPE")? as usize;if n==0||n%2!=0||n>1_000_000{return Err("SHAPE");}let h=(b-a)/n as f64;let mut s=eval1(e,&v,a)?+eval1(e,&v,b)?;for i in 1..n{s+=(if i%2==0{2.0}else{4.0})*eval1(e,&v,a+i as f64*h)?;}finite(s*h/3.0)}
fn ode(args:&[Value],rk4:bool)->Result<Value,&'static str>{if args.len()!=5{return Err("ARG");}let(e,v)=expr(&args[0])?;let mut x=num(&args[1])?;let mut y=num(&args[2])?;let x1=num(&args[3])?;let n=args[4].as_u64().ok_or("TYPE")? as usize;if n==0||n>1_000_000{return Err("LIMIT");}let h=(x1-x)/n as f64;for _ in 0..n{if rk4{let k1=eval2(e,&v,x,y)?;let k2=eval2(e,&v,x+h/2.0,y+h*k1/2.0)?;let k3=eval2(e,&v,x+h/2.0,y+h*k2/2.0)?;let k4=eval2(e,&v,x+h,y+h*k3)?;y+=h*(k1+2.0*k2+2.0*k3+k4)/6.0;}else{y+=h*eval2(e,&v,x,y)?;}x+=h;if !x.is_finite()||!y.is_finite(){return Err("NONFINITE");}}finite(y)}
...
fn richardson(args:&[Value])->Result<Value,&'static str>{if args.len()!=3{return Err("ARG");}let(e,v)=expr(&args[0])?;let x=num(&args[1])?;let h=num(&args[2])?;if h<=0.0{return Err("DOMAIN");}let d1=(eval1(e,&v,x+h)?-eval1(e,&v,x-h)?)/(2.0*h);let h2=h/2.0;let d2=(eval1(e,&v,x+h2)?-eval1(e,&v,x-h2)?)/(2.0*h2);finite((4.0*d2-d1)/3.0)}
