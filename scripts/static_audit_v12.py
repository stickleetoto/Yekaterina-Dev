"""Yekaterina v1.2 static audit.

Relationship to ``scripts/static_audit.py``:

* The v1.0.0 and v1.1 audits are preserved unmodified as frozen historical
  artifacts and are NOT edited by v1.2. This script asserts both facts by hash.
  Neither of them can pass on this tree, and that is correct: each one gates the
  operation count of its own release line, and v1.2 added operations.
...
* Every v1.1 gate is reproduced here, including the concurrency gates: unsafe
  code, thread spawn containment, fail-closed operation-safety classification,
  worker-count default, and benchmark/determinism infrastructure.
* v1.2 raises the operation count to 1284. The 1,215 operations of the v1.1 line
  are separately proven still present, unrenamed and in order by
  scripts/validate_full_audit.py against full_audit/opcodes_v11_frozen.json.
...
# The v1.1 audit, pinned for the same reason.
FROZEN_V11_AUDIT_SHA256 = "f3806bb52f9e74a28b82604546e4fb299afcdfd0312a8035a84f60db525422a8"
...
def check_v11_audit_preserved() -> None:
    p = ROOT / "scripts" / "static_audit_v11.py"
    if not p.exists():
        fail("scripts/static_audit_v11.py was deleted; the v1.1 audit must be preserved")
        return
    got = sha256_text(p.read_text(encoding="utf-8"))
    if got != FROZEN_V11_AUDIT_SHA256:
        fail(f"scripts/static_audit_v11.py was modified: {got}")
    else:
        ok("v1.1 static audit preserved unmodified (hash verified)")
...
def check_registry(registry: str, all_src: str) -> list[str]:
    ...
    if len(ops) != EXPECTED_OPS:
        ...
        ok(f"{len(ops)} unique registered opcodes ({V11_OPS} from the v1.1 line plus {len(ops) - V11_OPS} added by v1.2)")
...
def check_source_integrity() -> None:
    """Verify SOURCE_INTEGRITY_V12.txt against the tree it claims to pin.
    ...
    if not record.is_file():
        pending("SOURCE_INTEGRITY_V12.txt not written yet")
        return
    entries = []
    ...
    if not entries:
        fail("SOURCE_INTEGRITY_V12.txt lists no files")
        return
    drifted, absent = [], []
    for expected, rel in entries:
        ...
        if not f.is_file():
            ...
            drifted.append(rel)
    if absent:
        fail(f"SOURCE_INTEGRITY_V12.txt pins files that do not exist: {absent}")
    elif drifted:
        fail(f"source integrity drift in {drifted}; "
             "regenerate with scripts/gen_source_integrity_v12.py and commit both")
    else:
        ok(f"SOURCE_INTEGRITY_V12.txt verifies {len(entries)} files against the tree")
    ...
    if not v10.is_file():
        ...
        fail("SOURCE_INTEGRITY_V12.txt does not pin the v1.0.0 record")
    else:
...
def main() -> None:
    ...
    print("=" * 62)
    print(" Yekaterina v1.2 static audit")
    print("=" * 62)
    for m in _passes:
        print(f"PASS: {m}")
    for m in _pending:
